...
while True:
    try:
        ...
        bread_choice = pyip.inputMenu(['wheat', 'white', 'sourdough'], 'Would you like wheat, white, or sourdough bread?\n')
        ...
        protein_choice = pyip.inputMenu(['chicken', 'turkey', 'ham', 'tofu'], 'Would you like chicken, turkey, ham, or tofu?\n')
        ...
        if cheese == 'yes':
            cheese_choice = pyip.inputMenu(['cheddar', 'Swiss', 'mozzarella'], 'Would you like cheddar, Swiss, or mozzarella?\n', caseSensitive=False)
            if cheese_choice == 'Swiss':
                cost += 1.25
            else:
                cost += 1.00
        #Do they want to add lettuce, tomato, mayo, mustard?
        condiment = pyip.inputYesNo('Would you like to add mayo, mustard, lettuce or tomato?\n')
        if condiment == "yes": cost += .50
